...
//    });
    ...
    $(".formulario_contato .enviar").click(function () {
        $.ajax({
            url: 'php/contato.php',
            type: 'POST',
            data: $('.formulario_contato').serialize(),
            beforeSend: function () {
                $('.resp').html('<div class="resposta email-aguarde email-nao-validado"><span>Enviando seus dados...</span><span><img src="assets/images/ajax-loader.gif" alt=""/></span></div>').fadeIn(3000);
    ...
    $(".formulario_reserva_topo .enviar").click(function () {
        $.ajax({
            url: 'php/agendamento_mesa.php',
            type: 'POST',
            data: $('.formulario_reserva_topo').serialize(),
            beforeSend: function () {
                $('.resp-reserva').html('<div class="resposta email-aguarde email-nao-validado"><span>Enviando seus dados...</span><span><img src="assets/images/ajax-loader.gif" alt=""/></span></div>').fadeIn(3000);
            },
            success: function (valor) {
                $('.resp-reserva').html(valor).fadeIn(3000);
                //verifica a class depois de enviado, se tiver a class limpa todos os campos
                if ($(valor).hasClass('email-enviado')) {
                    $('.formulario_reserva_topo').each(function () {
                        this.reset();
                    });
                }
            }
...
$(function () {
    $.ajax({
        ...
        dataType: 'jsonp',
        cache: false,
        url: 'https://api.instagram.com/v1/users/self/media/recent/?access_token=5759066305.47476b4.b0684d7e3091430da9a0f267057173c7',
        success: function (data) {
            for (var i = 0; i < 5; i++) {
                $('.instafeed3').append('<li><a href="' + data.data[i].link + '"  target="_blank" style="background-image: url(' + data.data[i].images.low_resolution.url + ')"></a></li>');
            }
            var largura = $('.instafeed3 li').outerWidth();
            $('.instafeed3 li a').css('height', largura);
        }
    });
});
...
var marcarPagina = function () {
    var paginaMarcada = sessionStorage.getItem('pagina');
    if (paginaMarcada) {
        $('.menu-topo li').removeClass('active');
        $('.menu-topo li').each(function () {
            var pagina = $(this).data('pagina');
            if (pagina === paginaMarcada) {
                $(this).addClass('active');
            }
        });
